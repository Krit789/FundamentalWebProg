...
if ($_SERVER["REQUEST_METHOD"] == 'POST' && isset($_POST['action'])) {
    if (isset($_POST["e_id"]) && isset($_POST["firstname"]) && isset($_POST["lastname"]) && isset($_POST["address"]) && isset($_POST["email"]) && isset($_POST["phone"])) {
        if ($_POST['action'] == "SAVE") {
            ...
            if (isset($_SESSION['e_id']) && isset($_SESSION['firstname']) && isset($_SESSION['lastname']) && isset($_SESSION['address']) && isset($_SESSION['email']) && isset($_SESSION['phone'])) {
                $show = "yes";
            } else {
                $show = "error";
            }
    ...
    <div class="container my-4">
        ...
        <form method="POST">
            ...
            <div class="mb-3">
                ...
                <input class="form-control" type="number" value="<?php echo ($show == 'yes') ? $_SESSION['e_id'] : '' ?>" name="e_id">
            </div>
            <div class="mb-3">
                <div class="row">
                    <div class="col">
                        <label for="exampleFormControlInput1" class="form-label">Firstname</label>
                        <input class="form-control" type="text" value="<?php echo ($show == 'yes') ? $_SESSION['firstname'] : '' ?>" name="firstname">
                    </div>
                    <div class="col">
                        <label for="exampleFormControlInput1" class="form-label">Lastname</label>
                        <input class="form-control" type="text" value="<?php echo ($show == 'yes') ? $_SESSION['lastname'] : '' ?>" name="lastname">
                    </div>
                </div>
            </div>
            <div class="mb-3">
                <label for="exampleFormControlInput1" class="form-label">Address</label>
                <textarea class="form-control" rows="2" name="address"><?php echo ($show == 'yes') ? $_SESSION['address'] : '' ?></textarea>
            </div>
            <div class="mb-3">
                <label for="exampleFormControlInput1" class="form-label">Email</label>
                <input class="form-control" type="email" value="<?php echo ($show == 'yes') ? $_SESSION['email'] : '' ?>" name="email">
            </div>
            <div class="mb-3">
                <label for="exampleFormControlInput1" class="form-label">Phone</label>
                <input class="form-control" type="tel" value="<?php echo ($show == 'yes') ? $_SESSION['phone'] : '' ?>" name="phone">
            </div>
